...
class ModelGenerator:
    ...
    def prepare_column_type(self, column_data: Dict) -> str:
        ...
        column_type = types_mapping.get(column_data_type, type_not_found)
    ...
    def setup_column_attributes(self, column_data: Dict, table_pk: List[str], column: str) -> str:
    ...
    def generate_model(self,
        ...
        if table.get("table_name"):
            ...
            model = gt.model_template.format(
                model_name=create_model_name(table["table_name"], singular, exceptions),
    ...
    def create_header(self, tables: List[Dict]) -> str:
        ...
        if self.postgresql_dialect_cols:
            header += (
                gt.postgresql_dialect_import.format(types=",".join(self.postgresql_dialect_cols))
                + "\n"
            )
        if self.constraint:
